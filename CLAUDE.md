...
- **Rating**: `.star-rating` renders a dim `★★★★★` background with a colored `★★★★★` copy (`.star-rating-fill`, a real child `<span>`, absolutely positioned on top and clipped via an inline `width:NN%`) — a true partial-character fill (so half-star widths look right), not a special half-star glyph. The fill's width is set directly (inline `style.width`/attribute), not via a CSS custom property — iOS/iPadOS home-screen Safari (WKWebView) has a known bug where a custom property feeding a pseudo-element's `width` doesn't reliably resolve, which used to render every rated record's badge as a full 5 stars. The edit form's `#rf-rating` range input carries half-star units (`0`–`10`); `rating = value / 2`.
